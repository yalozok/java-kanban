...
class TaskTest {
    ...
    public void shouldUpdateCorrectName() {
        ...
        Assertions.assertEquals("Помыть собаку", task.getName(), "Task получает неверное имя");
    }

    @Test
    public void shouldUpdateCorrectDescription() {
        task.setDescription("С шампунем");
        Assertions.assertEquals("С шампунем", task.getDescription(), "Task получает неверное описание");
    }

    @Test
    public void shouldUpdateCorrectStatus() {
        task.setStatus(TaskStatus.DONE);
        Assertions.assertEquals(TaskStatus.DONE, task.getStatus(), "Task получает неверный статус");
    }
    ...
    public void shouldBeEqualsIfIdsEquals() {
        ...
        Assertions.assertNotEquals(task, taskToCompare, "Task с одинаковым id должны быть равны");
    }
}